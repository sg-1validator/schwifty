...
func SimulateMsgUpdateCollection(
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simtypes.Account, chainID string,
		...
		var (
			...
			allCollection = k.GetAllCollection(ctx)
			found         = false
		)
		for _, obj := range allCollection {
		...
		if !found {
			return simtypes.NoOpMsg(types.ModuleName, msg.Type(), "collection creator not found"), nil, nil
		}
